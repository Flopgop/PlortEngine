...
import net.flamgop.plort.engine.renderer.PlortCommandPool;
...
import net.flamgop.plort.engine.text.json.JsonAtlasFile;
import net.flamgop.plort.engine.text.json.JsonBounds;
import net.flamgop.plort.engine.text.json.JsonGlyph;
...
public class Atlas extends TrackedCloseable {
    ...
    public float stringWidth(String text, float scale) {
        ...
        for (int i = 0; i < numChars; i++) {
            ...
            Glyph glyph = glyphs.getOrDefault(c, glyphs.get('?'));
    ...
    public int calculateTextByteSize(List<Text> text) {
        int sum = 0;
        for (Text t : text) {
            sum += t.text().replaceAll("[\\r\\n]", "").length();
        }
        return sum * GLYPH_MESHLET_SIZE;
    }

    public PlortBuffer buildTextBuffer(List<Text> text) {
        int totalSize = calculateTextByteSize(text);
