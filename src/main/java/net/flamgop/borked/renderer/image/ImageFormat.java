...
public enum ImageFormat {
    ...
    private static final Map<Integer, ImageFormat> lookup = new HashMap<>();

    static {
        for (ImageFormat format : ImageFormat.values()) {
            lookup.put(format.vkQualifier, format);
        }
    }

    public static ImageFormat valueOf(int value) {
        ImageFormat format = lookup.get(value);
        if (format == null) {
            throw new IndexOutOfBoundsException("Unknown image format value: " + value);
        }
        return format;
    }
}
