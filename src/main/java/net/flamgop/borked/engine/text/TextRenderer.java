...
import net.flamgop.borked.engine.util.ResourceHelper;
...
public class TextRenderer extends TrackedCloseable {
    ...
    private static final int STRING_DATA_BYTES = 4 * 4 * Float.BYTES + 4 * Integer.BYTES;
    ...
    public TextRenderer(PlortDevice device, PlortSwapchain swapchain, PlortRenderPass renderPass, int maxFramesInFlight) {
        ...
        PlortDescriptorSet descriptorSet = new PlortDescriptorSet(
                new PlortDescriptor(PlortDescriptor.Type.STORAGE_BUFFER, 1, PlortShaderStage.Stage.MESH.bit()),
        ...
        this.pool = new PlortBufferedDescriptorSetPool(device, List.of(descriptorSet), maxFramesInFlight);
        pool.label("Text Buffered Descriptor Pool");
        ByteBuffer textShader = ResourceHelper.loadFromResource("text.spv");
        ...
        this.pipeline = PlortPipeline
                ...
                .pushConstant(new PlortPushConstant(0, STRING_DATA_BYTES, PlortShaderStage.Stage.ALL.bit()))
                .descriptorSetPool(pool.pool())
                .blendState(PlortBlendState.alphaBlend())
                .depthStencilStateInfo(new PlortDepthStencilState(
                        true, false, CompareOp.LESS, false, false, new PlortDepthStencilState.StencilOpState(), new PlortDepthStencilState.StencilOpState(), 0, 0
                ))
                .build();
    ...
    public void renderTextBuffer(VkCommandBuffer cmdBuffer, Atlas atlas, PlortBuffer buffer, int frame) {
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            writes.get(1)
                    ...
                    .descriptorType(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER)
                    .descriptorCount(1)
                    .pBufferInfo(bufferInfos.slice(1, 1));
            ...
            stringData.putInt(4 * 4 * Float.BYTES, glyphCount);
            stringData.putInt(4 * 4 * Float.BYTES + Integer.BYTES, 0);
            stringData.putInt(4 * 4 * Float.BYTES + 2 * Integer.BYTES, 0);
            stringData.putInt(4 * 4 * Float.BYTES + 3 * Integer.BYTES, 0);
            vkCmdPushConstants(cmdBuffer, pipeline.layout(), PlortShaderStage.Stage.ALL.bit(), 0, stringData);
